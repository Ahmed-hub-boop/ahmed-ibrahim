...
window.onscroll = function (){
    ...
    let proj_sec = document.querySelector(".prog-cont");
    for(let i = 0; i < progs.length;i++){
        //console.log(i)
        let p = document.createElement("div");
        p.className="prog-card";
        p.innerHTML=`
    <div class="pgc-img">
    <img src="${progs[i].img_src}">
    </div>
    <div class="pgc-data">
    <span class="pgc-name">${progs[i].name}</span>
    <div class="pgc-links">
    <a href="${progs[i].img_src}" target="_blank">View Photo</a>
    <a href="${progs[i].prog_link}" target="_blank">Live Preview</a>
    </div>
    </div>`;
        proj_sec.appendChild(p);
    }